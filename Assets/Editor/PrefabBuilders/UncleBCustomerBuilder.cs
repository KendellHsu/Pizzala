...
namespace Pizzala.EditorTools
{
    ...
    //   3. CustomerController.idle/walk/throwAnimatorController = uncleB standing / walking / throwing
    //      (與 Soldier 相同機制:走動切 walk、丟回切 throw 控制器,clip 靠骨架路徑對應)
    //   4. SaveAsPrefabAssetAndConnect 存成 Assets/Prefabs/PZ_Customer_UncleB.prefab
    //      (存到跟基底不同的路徑,Unity 會自動建立 Variant 關係)
    //   5. 把目前開啟場景 CustomerSpawner 的 customerPrefabs 設為
    ...
    public static class UncleBCustomerBuilder
    {
        ...
        const string ThrowControllerPath = "Assets/Art/NPC/UncleB/Animator Controller_uncleB_throwing.controller";
        ...
        public static void Build()
        {
            ...
            var throwCtrl = AssetDatabase.LoadAssetAtPath<RuntimeAnimatorController>(ThrowControllerPath);
            if (basePrefab == null)
            ...
            try
            {
                ...
                // 接 CustomerController:idle/walk/throw 控制器
                cc.idleAnimatorController = idleCtrl;
                cc.walkAnimatorController = walkCtrl;
                cc.throwAnimatorController = throwCtrl; // 缺檔時為 null,丟回時就不換動畫
                if (throwCtrl == null)
                    Debug.LogWarning($"UncleBCustomerBuilder: 找不到丟回動畫控制器 {ThrowControllerPath},UncleB 丟回時不會有出手動畫。");

...
**驗收目標**
- [ ] GameManager 的 `throwbackPrefab` 拖入後,故意讓客人超時:客人先閃紅約 0.8 秒,然後披薩朝玩家飛來
- [ ] 站著不動會被砸中 → 螢幕糊醬汁(需 #8 完成)
- [ ] 預警瞬間側跨一步或下蹲,披薩**會落空**,砸在身後牆上並留下髒污
- [ ] 落空的披薩 1.5 秒後消失,5 秒後保底自毀,場上不會堆積
- [ ] 玩家自己丟的披薩碰到自己的頭**不會**觸發任何判定
...
**驗收目標**
- [ ] 連續亂丟 10 顆披薩到牆面、地板、桌面:每個失誤落點都出現髒污
- [ ] 髒污**貼合表面**且無 z-fighting 閃爍(有閃爍 → 調 DirtManager 的 `surfaceOffset`)
- [ ] 髒污形狀有隨機變化(旋轉、大小、圖案都會隨機)
- [ ] 丟同一個位置 10 次,幀率無明顯下降(Quest Link 下 72fps 不掉)
...
**驗收目標**
- [ ] Play 後三座各出現一顆披薩,口味外觀正確
- [ ] 拿走披薩丟出去,約 1.5 秒後自動補新的一顆
- [ ] 補的披薩口味永遠正確(Spawner 會覆寫 `flavor`)
- [ ] 把披薩拿起來又放回原位不丟,不會瘋狂重複生成
- [ ] 連續丟 20 顆,場上舊披薩留在原地當髒污、不影響幀率
